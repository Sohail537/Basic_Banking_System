...
router.post('/adduser',(req, res) =>{ const {userName, userEmail, userNumber, userAmount}  = req.body;
    ...
    User.save().then(()=>{ res.render('addUser', {title: "Add User", msg:'User Added Successfully'})}).catch((err)=>{
        console.log(err)
    })
})
    ...
    if(reciverName === 'Select Reciver Name' || reciverEmail === 'Select Reciver Email'){    
        ...
        Promise.all([Sender,Reciver]).then(([senderData,reciverData]) => {
            senderData.forEach( async (c) => {
                ...
                history.save().then((r)=>{}).catch(err => {console.log(err)});
